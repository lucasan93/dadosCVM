...
cad_fi <- function() {
  ...
  read.csv(temp,
    ...
    mutate(tipo            = as.factor(.data$tipo),
           cnpj            = as.character(.data$cnpj),
           nome            = as.character(.data$nome),
           registro        = as.Date(.data$registro,
                                     '%Y-%m-%d'),
           constituicao    = as.Date(.data$constituicao,
                                     '%Y-%m-%d'),
           codigo_cvm      = as.character(.data$codigo_cvm),
           cancelamento    = as.Date(.data$cancelamento,
                                     '%Y-%m-%d'),
           situacao        = as.factor(.data$situacao),
           inicio_sit      = as.Date(.data$inicio_sit,
                                     '%Y-%m-%d'),
           inicio_atv      = as.Date(.data$inicio_atv,
                                     '%Y-%m-%d'),
           inicio_exr      = as.Date(.data$inicio_exr,
                                     '%Y-%m-%d'),
           fim_exr         = as.Date(.data$fim_exr,
                                     '%Y-%m-%d'),
           classe          = as.factor(.data$classe),
           dt_ini_classe   = as.Date(.data$dt_ini_classe,
                                     '%Y-%m-%d'),
